  ...
  function renderResults(q) {
    ...
    if (!q || q.trim().length === 0) {
      resultsContainer.innerHTML = `
        <div class="search-empty-state">
          Type to search across all 28 States, 8 UTs, and authentic curated destinations across Bharat.
        </div>
      `;
      currentResults = [];
      selectedIndex = -1;
    ...
    resultsContainer.innerHTML = currentResults.map((item, idx) => `
      <div class="search-result-item ${idx === 0 ? 'selected' : ''}" data-idx="${idx}" data-route="${item.route}">
        <img src="${item.image}" alt="${item.title}" class="search-result-img" loading="lazy" onerror="this.src='assets/images/destinations/fallback-raahi.jpg'" />
